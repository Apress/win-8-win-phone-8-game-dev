...
namespace CosmicRocksPartIII_Win8
{
    internal class SpaceshipObject : GameFramework.SpriteObject
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            // Assume we're not thrusting...
            _isThrusting = false;

            // Process touch input?
            if (GameHelper.GetInputCapabilities().IsTouchPresent)
            {
                Update_ProcessTouchInput();
            }
            // Process keyboard input?
            if (GameHelper.GetInputCapabilities().IsKeyboardPresent)
            {
                Update_ProcessKeyboardInput();
            }
            // Process gamepad input?
            if (GameHelper.GetInputCapabilities().IsGamePadPresent)
            {
                Update_ProcessGamepadInput();
            }

            // If we actually aren't thrusting, stop the thrust sound if it's playing
            if (!_isThrusting)
            {
                StopThrusterSound();
            }
        ...
        private void Update_ProcessGamepadInput()
        {
            // Read the gamepad state
            GamePadState gamepadState = GamePad.GetState(PlayerIndex.One);

            // If the dpad left/right has been pressed, rotate the ship at a fixed speed.
            // The value here can be varied to change how quickly the ship rotates.
            if (gamepadState.DPad.Left == ButtonState.Pressed) _targetAngle -= MathHelper.ToRadians(RotationSpeed);
            if (gamepadState.DPad.Right == ButtonState.Pressed) _targetAngle -= MathHelper.ToRadians(-RotationSpeed);

            // If the left thumbstick has been moved left or right, rotate the ship according
            // to the distance the stick has moved. Once again we scale the input amount by
            // a speed to control how quickly the ship rotates.
            if (gamepadState.ThumbSticks.Left.X != 0) _targetAngle += MathHelper.ToRadians(gamepadState.ThumbSticks.Left.X * RotationSpeed);

            // If the player is pressing either of the trigger buttons, thrust the ship
            if (gamepadState.IsButtonDown(Buttons.LeftTrigger) || gamepadState.IsButtonDown(Buttons.RightTrigger)) Thrust();

            // If the player is pressing the B button, hyperspace
            if (gamepadState.IsButtonDown(Buttons.B)) Hyperspace();

            // If the player is pressing the A button, fire a bullet
            if (gamepadState.IsButtonDown(Buttons.A))
            {
                // Was the first button pressed during the last update too? If so, ignore this button press
                if (!_lastGamePadState.IsButtonDown(Buttons.A))
                {
                    // Fire a bullet
                    FireBullet();
                }
            ...
            // Remember the gamepad state for next time so that we can detect button down/up events
        ...
        private void Thrust()
        {
            ...
            StartThrusterSound();
            // Remember we're thrusting so that the sound effect can be allowed to continue to play
            _isThrusting = true;
        }
